...
const TrafficControl: React.FC<TrafficControlProps> = ({ 
  ...
  const [editDeliveryModal, setEditDeliveryModal] = useState<{roId: string, itemIdx: number, delIndex: number, delivery: any} | null>(null);
  ...
  const allDeliveries = useMemo(() => {
    ...
    (history || []).forEach(ro => {
      ...
      items.forEach((item: any, itemIndex: number) => {
        if (item.deliveries) {
          (item.deliveries || []).forEach((del: any, delIndex: number) => {
  ...
  const handleSaveEditDelivery = () => {
    if (!editDeliveryModal || receiveQty <= 0) return;

    const { roId, itemIdx, delIndex } = editDeliveryModal;
    const ro = history.find(h => h.id === roId);
    if (!ro) return;
    ...
    const oldQty = deliveries[delIndex].quantity;
    const qtyDiff = receiveQty - oldQty;
    ...
    item.receivedQuantity = (item.receivedQuantity || 0) + qtyDiff;
  ...
  const handleDeleteDelivery = (roId: string, itemIdx: number, delIndex: number) => {
    if (!window.confirm('Apakah Anda yakin ingin menghapus data riwayat parsial ini?')) return;
    ...
    const deletedQty = deliveries[delIndex].quantity;
    deliveries.splice(delIndex, 1);

    item.receivedQuantity = Math.max(0, (item.receivedQuantity || 0) - deletedQty);
    item.deliveries = deliveries;
  ...
  return (
    <div className="py-6 md:p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="flex bg-white p-2 rounded-[28px] border border-slate-100 shadow-sm w-full lg:w-fit overflow-x-auto no-scrollbar">
          {[
                            ...
                            <tr key={del.id} className="hover:bg-slate-50/50 transition-colors">
                               ...
                               <td className="px-6 py-6 text-center">
                                  <div className="flex items-center justify-center gap-2">
                                     <button 
                                       onClick={() => {
                                         setReceiveQty(del.quantity);
                                         setActualArrivalDate(normalizeDate(del.date));
                                         setEditDeliveryModal({
                                           roId: del.roId,
                                           itemIdx: del.itemIndex,
                                           delIndex: del.delIndex,
                                           delivery: del
                                         });
                                       }}
                                       className="w-8 h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors"
                                       title="Edit"
                                     >
                                        ✏️
                                     </button>
                                     <button 
                                       onClick={() => handleDeleteDelivery(del.roId, del.itemIndex, del.delIndex)}
                                       className="w-8 h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center hover:bg-rose-50 hover:text-rose-600 transition-colors"
                                       title="Hapus"
                                     >
                                        🗑️
                                     </button>
      ...
      {editDeliveryModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4">
           <div className="bg-white rounded-[40px] p-10 max-w-lg w-full shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-3xl mb-6">✏️</div>
              <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-none mb-2">Edit Riwayat Parsial</h3>
              <p className="text-sm text-slate-500 mb-8 font-medium">Perbarui tanggal kedatangan atau jumlah barang yang diterima.</p>
              ...
              <div className="space-y-6">
                 ...
                 <div className="bg-[#1C0770] p-8 rounded-[32px] shadow-2xl shadow-indigo-100">
                    <label className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-4 text-center">Jumlah Barang Diterima</label>
                    <div className="relative">
                       <input 
                         type="number"
                         autoFocus
              ...
              <div className="flex gap-4 mt-10">
                 <button onClick={() => setEditDeliveryModal(null)} className="flex-1 py-5 text-slate-400 font-bold hover:bg-slate-50 rounded-2xl transition text-xs uppercase tracking-widest">Batal</button>
                 <button onClick={handleSaveEditDelivery} className="flex-1 py-5 bg-[#1C0770] text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all">Simpan Perubahan</button>
              </div>
           </div>
        </div>
      )}
